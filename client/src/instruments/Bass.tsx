...
function Bass({ synth, setSynth }: InstrumentProps): JSX.Element {
  ...
  const strings = List([
    {position: 0, firstOctave: 3, keyOffset: 7}, // firstNote: 'G'
    {position: 1, firstOctave: 2, keyOffset: 2}, //firstNote: 'D'
    {position: 2, firstOctave: 2, keyOffset: 9}, //firstNote: 'A',
    {position: 3, firstOctave: 2, keyOffset: 4}, //firstNote: 'E',
  ]);
          ...
          {strings.map(
            string =>
              Range(0, 22).map(
                fret =>
                  {
                    const localKey = keys[(string.keyOffset + fret) % 12];
